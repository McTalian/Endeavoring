...
You are GPT-5.1-Codex-Max performing a precise code refactoring.

## Context
[Provide relevant context about the codebase, architecture, conventions]
...
4. **Report to user**:
   ...
   Ready for Step N+1? (or type /test to validate in-game first)
   ```
...
**Validation**: Grep for old name should return zero results
...
2. Move/create files in new locations (empty or copied)
3. Update imports to new locations
4. Migrate functionality in dependency order
5. Clean up old files
6. Update TOC/imports

**Validation**: Old directory should be empty and removable
...
**User**: `/refactor Extract Protocol module from Sync.lua`

**Your Response**:
1. Ask clarifying questions about scope and dependencies
2. Analyze Sync.lua and identify protocol-related code
3. Ask user to create git checkpoint
4. Present step-by-step plan (e.g., create file, extract functions, update call sites, cleanup)
5. For each step, use `runSubagent` with GPT-5.1-Codex-Max to execute changes
6. Validate after each step (syntax, build, grep checks)
7. Final validation and recommend runtime testing
