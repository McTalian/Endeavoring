...
4. **Proper event chain**: PEW → ViewActiveNeighborhood → RunNextFrame(RequestPlayerHouses) → PLAYER_HOUSE_LIST_UPDATED → RequestInitiativeInfo → NEIGHBORHOOD_INITIATIVE_UPDATED → Data loads ✓
...
- ✅ Settings gear button appears in main frame header
- ✅ Button click opens settings panel correctly
- ✅ Button tooltip displays properly
- ✅ Housing Dashboard corruption RESOLVED - tested multiple times:
  - Login → Open Housing Dashboard → Teleport to neighborhood → Check Endeavors tab
  - Data now loads correctly, no blank pages
  - Both Blizzard's UI and Endeavoring show proper initiative data
- ✅ No Lua errors on login, zone transitions, or addon operations
- ✅ Endeavoring button still appears in Housing Dashboard (after user opens it)

### Known Issues Resolved
- ✅ Housing Dashboard blank Endeavors tab after zone-in (FIXED)
- ✅ "Try opening the housing dashboard" errors reported by users (FIXED)
- ⚠️ Previous session's "Remember Last Tab" bug may still exist (not retested this session)
...
2. **Test "Remember Last Tab" bug** from previous session:
   - The bug: Tab always saved regardless of checkbox state
   - Root cause: SaveLastTab() called unconditionally in SetTab hook
   - Potential fix: Check `settings.rememberLastTab` before calling `DB.SetLastSelectedTab()`
   - File: Core.lua (lines 42-46)

3. **Update version tags for beta**:
   - Remove `--@alpha@` guards if appropriate
   - Update TOC version
   - Update internal version strings
   - Tag git commit as beta release

4. **Beta release documentation**:
...
1. Removed force-loading → Still reproduced
2. Added debug logging → Discovered data was nil/empty
3. Checked viewing neighborhood state → Found it wasn't set
4. Added ViewActiveNeighborhood() → Mostly fixed
5. Added RunNextFrame delay → Completely fixed
...
- Test "Remember Last Tab" functionality (potential bug from Feb 11 session)
...
1. ✅ Improved UX with convenient settings button
2. ✅ Diagnosed and fixed critical stability issue
3. ✅ Discovered important Blizzard API requirements (viewing context)
4. ✅ Eliminated force-loading fragility
5. ✅ Positioned addon for beta release
